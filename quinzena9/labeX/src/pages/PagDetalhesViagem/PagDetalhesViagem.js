...
import CartaCandidato from '../../components/CartaCandidato/CartaCandidato'
import { TripDataContainer, TripScreenContainer } from "./styled"

const PagDetalhesViagem = () => {
    useProtectedPage()
    ...
    const candidates = tripDetails && tripDetails.trip && tripDetails.trip.candidates.map((c) => {
        return <CartaCandidato key={c.id} candidate={c} tripId={id} getTripDetails={getTripDetails} />
    })
    ...
    return (
        <TripScreenContainer>
            ...
            <button onClick={() => goToPagInicioAdm(history)}>Voltar</button>
...
export default PagDetalhesViagem
